...
// --- VARIABLES PARA TICKET (Creación) ---
...
// =================================================================
// GESTIÓN MANTENCIONES GLOBAL (Con Botón Reimprimir)
// =================================================================
async function handleBuscarMantenciones(e) {
    ...
    try {
        ...
        data.forEach(m => {
            ...
            let fechaVis = m.fecha;
            if(m.fecha && m.fecha.includes('-')) { const [y,mm,d] = m.fecha.split('-'); fechaVis = `${d}-${mm}-${y}`; }
            
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td><strong>${fechaVis}</strong></td>
                <td>${m.patente} <br><small style="color:gray;">${m.marca}</small></td>
                <td>${m.cliente_nombre}</td>
                <td style="max-width:300px;">${m.trabajos_realizados}</td>
                <td>
                    <button class="btn-descargar" onclick='window.reimprimirTicket(${JSON.stringify(m)})' title="Imprimir Ticket" style="padding: 0.3rem 0.6rem; margin-right: 5px;"><i class="fa-solid fa-print"></i></button>
                    ...
                    <button class="btn-delete" onclick="window.deleteMantenimiento(${m.id})" title="Eliminar"><i class="fa-solid fa-trash"></i></button>
                </td>`;
...
// Función auxiliar para reimprimir desde el historial
window.reimprimirTicket = (m) => {
    const datosTicket = {
        ...m,
        cliente: m.cliente_nombre, // Adaptar nombres de campos
        modelo: `${m.marca} ${m.modelo}`
    };
    generarTicketTermico(datosTicket);
};
...
async function handleQuickMantencion(e) {
    ...
    setTimeout(() => {
        seleccionarVehiculo(data.datos.id, patente, `${data.datos.marca} ${data.datos.modelo}`);
...
function generarTicketTermico(datos) {
    ...
    ventana.document.write(`
        <html><head><title>Ticket GUDEX</title>
        <style>
            body { font-family: 'Courier New', monospace; width: 72mm; margin: 0; padding: 5px; font-size: 12px; color: #000; }
            ...
            <div class="header"><div class="logo">GUDEX SERVITECA</div><div>Comprobante de Servicio</div><div>${new Date().toLocaleString()}</div></div>
            <div class="info">
                <div><span class="label">Cliente:</span> ${datos.cliente}</div>
                <div><span class="label">Vehículo:</span> ${datos.modelo || ''}</div>
                <div><span class="label">Patente:</span> ${datos.patente}</div>
                <div><span class="label">Fecha:</span> ${datos.fecha}</div>
                <div><span class="label">KM:</span> ${datos.kilometraje}</div>
            </div>
            <div class="content">
                <div class="item"><div class="label">Trabajos:</div><div>${datos.trabajos_realizados}</div></div>
                ${datos.repuestos_usados ? `<div class="item"><div class="label">Repuestos:</div><div>${datos.repuestos_usados}</div></div>` : ''}
                <hr>
                ${datos.proximo_km_sugerido ? `<div class="item" style="margin-top:10px;font-size:13px;text-align:center;font-weight:bold;">PRÓXIMA: ${datos.proximo_km_sugerido} KM</div>` : ''}
                ${datos.proxima_sugerencia ? `<div class="item"><div class="label">Nota:</div><div>${datos.proxima_sugerencia}</div></div>` : ''}
            </div>
...
// =================================================================
// VEHÍCULOS Y MANTENIMIENTOS (FORMULARIO)
// =================================================================
window.seleccionarVehiculo = async (id, patente, modeloCompleto) => {
    vehiculoSeleccionadoId = id;
    
    // Guardar datos globales para impresión desde el formulario de creación
    if(patente) vehiculoSeleccionadoPatente = patente;
    if(modeloCompleto) vehiculoSeleccionadoModelo = modeloCompleto;
    ...
    hist.forEach(m => {
        ...
        li.innerHTML = `<span class="list-item-label"><strong>${m.fecha}</strong>: ${m.trabajos_realizados}</span><button class="btn-edit" onclick='window.openMantenimientoModal(${JSON.stringify(m)})'><i class="fa-solid fa-pen"></i></button>`;
...
async function handleAgregarMantenimiento(e) {
    ...
    // Verificar impresión
    if(document.getElementById('imprimir-ticket') && document.getElementById('imprimir-ticket').checked) {
        const nombreCliente = document.getElementById('detalle-cliente-nombre').textContent;
        generarTicketTermico({
            ...datos,
            cliente: nombreCliente,
            patente: vehiculoSeleccionadoPatente,
            modelo: vehiculoSeleccionadoModelo
        });
        document.getElementById('imprimir-ticket').checked = false;
    }

    alert('Registrado'); 
    formAgregarMantenimiento.reset(); 
    window.seleccionarVehiculo(vehiculoSeleccionadoId, '');
}

// --- RESTO DEL SISTEMA (Sin cambios en lógica, solo copiando para integridad) ---
...
function renderCitas(citas, lista, tipo) {
    ...
    citas.forEach(c => {
        ...
        li.innerHTML = `<p><strong>${fecha}</strong><br>${c.cliente_nombre} (${c.vehiculo_patente})<br><em style="color:var(--text-light);">${c.servicio_solicitado}</em></p>${botones}`;
...
window.gestionarCita = async (id, estado, clientId, vehId, cliName, pat, serv) => {
    ...
    cargarDashboardCitas();
    if(estado === 'Terminada' && confirm("¿Registrar en historial?")) {
        // Redirección forzada a la vista de clientes
        const navLinks = document.querySelectorAll('.nav-link');
        navLinks.forEach(l => l.classList.remove('active'));
        const clientLink = document.querySelector('[data-view="clientes"]');
        if(clientLink) { clientLink.classList.add('active'); clientLink.closest('.nav-group').classList.add('open'); }
        showView('clientes');
        await seleccionarCliente(clientId);
        setTimeout(() => {
             seleccionarVehiculo(vehId, pat, '');
             ...
             formAgregarMantenimiento.scrollIntoView({ behavior: 'smooth' });
             formAgregarMantenimiento.style.border = "2px solid var(--primary)";
             setTimeout(() => formAgregarMantenimiento.style.border = "1px solid #ccc", 3000);
        }, 600);
    }
};
async function handleAdminAgendarCita(e) {
    e.preventDefault();
    if (!clienteSeleccionadoId || !vehiculoSeleccionadoId) return alert('Selecciona Cliente y Vehículo.');
    ...
    if(res.ok) { alert('Cita agendada.'); formAdminAgendarCita.reset(); cargarDashboardCitas(); } else { alert('Error.'); }
}
...
async function cargarVehiculosCliente(id) {
    ...
    vehs.forEach(v => {
        const li = document.createElement('li'); li.className = 'list-item';
        li.innerHTML = `<span class="list-item-label" onclick="window.seleccionarVehiculo(${v.id}, '${v.patente}', '${v.marca} ${v.modelo}')"><i class="fa-solid fa-car"></i> ${v.marca} ${v.modelo}</span><button class="btn-delete" onclick="window.deleteVehiculo(${v.id})"><i class="fa-solid fa-trash"></i></button>`;
...
window.deleteMantenimiento = async (id) => { if(confirm('¿Borrar?')) { await fetch(`${API_URL}/mantenimientos/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); handleBuscarMantenciones(null); }};
...
async function handleDescargarExcelVehiculos() {
    window.location.href = `${API_URL}/excel/download/vehiculos?token=${tokenGuardado}`;
}
...
async function handleRestaurarExcelVehiculos(e) {
    ...
    try {
        const res = await fetch(`${API_URL}/excel/restore/vehiculos?token=${tokenGuardado}`, {
            method: 'POST',
            body: formData
        });
        const data = await res.json();
        if (res.ok) {
            alert(data.message);
            fileInput.value = '';
            cargarConfigMarcas(); // Recargar la lista visual de marcas
        } else {
