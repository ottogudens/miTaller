...
function iniciarPanelAdmin() {
    // Acordeón
    document.querySelectorAll('.nav-toggle').forEach(t => t.addEventListener('click', (e) => {
...
window.gestionarCita = async (id, estado, clientId, vehId, cliName, pat, serv) => {
    ...
    if(estado === 'Terminada' && confirm("¿Registrar en historial?")) {
        // Navegación forzada a clientes
        document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active'));
        const clLink = document.querySelector('[data-view="clientes"]');
        if(clLink) { clLink.classList.add('active'); clLink.closest('.nav-group').classList.add('open'); }
        showView('clientes');
        ...
        setTimeout(() => {
             ...
             setTimeout(() => formAgregarMantenimiento.style.border = "1px solid var(--border)", 3000);
        }, 600);
    }
};
...
async function cargarVehiculosCliente(id) {
    ...
    vehs.forEach(v => {
        const li = document.createElement('li'); li.className = 'list-item';
        li.innerHTML = `<span class="list-item-label" onclick="window.seleccionarVehiculo(${v.id}, '${v.patente}')"><i class="fa-solid fa-car"></i> ${v.marca} ${v.modelo}</span><button class="btn-delete" onclick="window.deleteVehiculo(${v.id})"><i class="fa-solid fa-trash"></i></button>`;
...
async function cargarConfigMarcas() {
    ...
    marcas.forEach(m => {
        const li = document.createElement('li'); const nombre = m.nombre || m; const id = m.id;
        li.innerHTML = `<span>${nombre}</span> <button class="btn-delete" type="button"><i class="fa-solid fa-trash"></i></button>`;
        li.querySelector('button').onclick = (e) => { e.stopPropagation(); window.borrarMarca(id); };
        li.onclick = () => { seleccionarMarcaConfig(id, nombre); document.querySelectorAll('#lista-config-marcas li').forEach(l => l.classList.remove('selected')); li.classList.add('selected'); };
...
async function seleccionarMarcaConfig(id, nombre) { marcaSeleccionadaId = id; tituloModelosMarca.textContent = `Modelos de: ${nombre}`; document.getElementById('info-modelos-marca').style.display = 'none'; panelModelos.classList.remove('hidden'); cargarConfigModelos(id); }
async function cargarConfigModelos(id) { const res = await fetch(`${API_URL}/vehiculos-data/modelos-by-id/${id}`, { headers: { 'Authorization': `Bearer ${tokenGuardado}` }, cache: 'no-store' }); const mods = await res.json(); listaConfigModelos.innerHTML = mods.length ? '' : '<li>Sin modelos.</li>'; mods.forEach(m => { const li = document.createElement('li'); li.innerHTML = `<span>${m.nombre}</span> <button class="btn-delete" onclick="window.borrarModelo(${m.id})"><i class="fa-solid fa-trash"></i></button>`; listaConfigModelos.appendChild(li); }); }
async function handleCrearModelo(e) { e.preventDefault(); const nombre = document.getElementById('nuevo-modelo-nombre').value.trim(); if(!nombre || !marcaSeleccionadaId) return; await fetch(`${API_URL}/vehiculos-data/modelos`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${tokenGuardado}` }, body: JSON.stringify({ nombre, marca_id: marcaSeleccionadaId }) }); document.getElementById('nuevo-modelo-nombre').value = ''; cargarConfigModelos(marcaSeleccionadaId); }
...
// --- OTROS ---
async function handleBuscarPatente(e) { e.preventDefault(); const pat = document.getElementById('busqueda-patente').value.trim(); if(!pat) return; const res = await fetch(`${API_URL}/vehiculos/buscar/${pat}`, { headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); if(!res.ok) { resultadoBusqueda.classList.add('hidden'); return alert('No encontrado'); } const data = await res.json(); const v = data.datos; const h = data.historial||[]; document.getElementById('res-patente').textContent = v.patente; document.getElementById('res-marca-modelo').textContent = `${v.marca} ${v.modelo}`; document.getElementById('res-anio').textContent = v.anio; document.getElementById('res-cliente').textContent = v.cliente_nombre; document.getElementById('res-email').textContent = v.cliente_email; document.getElementById('res-telefono').textContent = v.cliente_telefono; btnIrCliente.onclick = () => { showView('clientes'); window.seleccionarCliente(v.cliente_id_real); }; const ul = document.getElementById('res-lista-historial'); ul.innerHTML = h.length ? '' : '<li>Sin historial.</li>'; h.forEach(m => { const li = document.createElement('li'); li.className = 'list-item'; li.innerHTML = `<p><strong>${m.fecha}</strong> (${m.kilometraje}km)<br>${m.trabajos_realizados}</p>`; ul.appendChild(li); }); resultadoBusqueda.classList.remove('hidden'); }
...
async function cargarDatosDeVehiculos() { const res = await fetch(`${API_URL}/vehiculos-data/marcas`, { headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); marcasData = await res.json(); poblarSelectMarcas(); }
...
async function poblarSelectModelos(marca) { selectModelo.innerHTML = '<option value="">-- Modelo --</option>'; if(!marca) return; const res = await fetch(`${API_URL}/vehiculos-data/modelos/${marca}`, { headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); const mods = await res.json(); mods.forEach(m => selectModelo.add(new Option(m, m))); }
