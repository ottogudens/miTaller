...
// HELPERS UX: Notificaciones SweetAlert2
const showSuccess = (title, text = '') => Swal.fire({ icon: 'success', title, text, timer: 2000, showConfirmButton: false });
const showError = (text) => Swal.fire({ icon: 'error', title: 'Error', text });
const showConfirm = async (title, text) => {
    const res = await Swal.fire({
        title, text, icon: 'warning',
        showCancelButton: true, confirmButtonColor: '#CC0000', cancelButtonColor: '#2c3e50',
...
function iniciarPanelAdmin() {
    ...
    // Event Listeners (Simplificados con nuevas funciones handle)
...
async function handleScanInventario(code) {
    ...
    } catch (e) { showError('Error al escanear producto'); }
    scannerInput.value = '';
}
async function handleUpdateStock(e) {
    ...
    showSuccess('Stock Actualizado'); panelExistente.classList.add('hidden'); scannerInput.focus();
}
async function handleCrearScan(e) {
    ...
    showSuccess('Producto Creado'); panelNuevo.classList.add('hidden'); document.getElementById('form-crear-scan').reset(); scannerInput.focus();
}
...
function handleScanBarcode(code) {
    ...
    if(prod) { itemsMantenimiento.push({ id: prod.id, nombre: prod.nombre, cantidad: 1, precio: prod.precio_venta, categoria: prod.categoria }); renderizarCarrito(); inputScanBarcode.value = ''; inputScanBarcode.focus(); } else { Swal.fire('No encontrado', 'El código no corresponde a un producto', 'warning'); inputScanBarcode.value = ''; }
}
...
async function handleCrearServicio(e) {
    ...
    await fetch(`${API_URL}/productos`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${tokenGuardado}` }, body: JSON.stringify(datos) });
    showSuccess('Servicio creado'); document.getElementById('form-crear-servicio').reset(); cargarServicios();
}
...
window.borrarCategoria = async(id) => { if(await showConfirm('¿Eliminar Categoría?', 'Esto no se puede deshacer')) { await fetch(`${API_URL}/categorias/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); cargarCategoriasPadres(); panelCatHijos.classList.add('hidden'); }};
...
async function handleAgregarMantenimiento(e) { e.preventDefault(); if(!vehiculoSeleccionadoId) return showError('Selecciona un vehículo primero'); const datos = { fecha: document.getElementById('admin-mantenimiento-fecha').value, kilometraje: document.getElementById('admin-mantenimiento-km').value, trabajos_realizados: document.getElementById('admin-mantenimiento-trabajos').value, repuestos_usados: document.getElementById('admin-mantenimiento-repuestos').value, proxima_sugerencia: document.getElementById('admin-mantenimiento-sugerencia').value, proximo_km_sugerido: document.getElementById('admin-mantenimiento-prox-km').value, vehiculo_id: vehiculoSeleccionadoId, items: itemsMantenimiento }; const res = await fetch(`${API_URL}/mantenimientos`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${tokenGuardado}` }, body: JSON.stringify(datos) }); if(res.ok) { if(document.getElementById('imprimir-ticket').checked) { const nombreCliente = document.getElementById('detalle-cliente-nombre').textContent; generarTicketTermico({ ...datos, cliente: nombreCliente, patente: vehiculoSeleccionadoPatente, modelo: vehiculoSeleccionadoModelo }); document.getElementById('imprimir-ticket').checked = false; } showSuccess('Mantenimiento Registrado'); formAgregarMantenimiento.reset(); itemsMantenimiento = []; renderizarCarrito(); cargarInventario(); window.seleccionarVehiculo(vehiculoSeleccionadoId, '', ''); } else showError('Error al registrar mantenimiento'); }
...
async function handleAgregarVehiculo(e) { e.preventDefault(); if(!clienteSeleccionadoId) return showError('Selecciona cliente'); const datos = { patente: document.getElementById('admin-patente').value, marca: document.getElementById('admin-marca').value, modelo: document.getElementById('admin-modelo').value, anio: document.getElementById('admin-anio').value, cliente_id: clienteSeleccionadoId }; if(!datos.marca) return showError('Marca requerida'); await fetch(`${API_URL}/vehiculos`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${tokenGuardado}` }, body: JSON.stringify(datos) }); showSuccess('Vehículo Agregado'); formAgregarVehiculo.reset(); poblarSelectModelos(''); cargarVehiculosCliente(clienteSeleccionadoId); }
async function handleDescargarFichaCliente() { if (!clienteSeleccionadoId) return showError('Selecciona cliente'); window.open(`${API_URL}/reports/cliente/${clienteSeleccionadoId}?token=${tokenGuardado}`, '_blank'); }
...
window.borrarMarca = async (id) => { if(await showConfirm('¿Borrar marca?', 'Se eliminarán también sus modelos.')) { await fetch(`${API_URL}/vehiculos-data/marcas/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); cargarConfigMarcas(); panelModelos.classList.add('hidden'); cargarDatosDeVehiculos(); }};
...
async function handleCrearModelo(e) { e.preventDefault(); const nombre = document.getElementById('nuevo-modelo-nombre').value.trim(); if(!nombre || !marcaSeleccionadaId) return showError('Selecciona marca'); await fetch(`${API_URL}/vehiculos-data/modelos`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${tokenGuardado}` }, body: JSON.stringify({ nombre, marca_id: marcaSeleccionadaId }) }); document.getElementById('nuevo-modelo-nombre').value = ''; cargarConfigModelos(marcaSeleccionadaId); }
window.borrarModelo = async (id) => { if(await showConfirm('¿Borrar Modelo?')) { await fetch(`${API_URL}/vehiculos-data/modelos/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); cargarConfigModelos(marcaSeleccionadaId); }};
...
async function handleRestaurarBackup(e) { e.preventDefault(); const file = document.getElementById('backup-file').files[0]; if(!file) return showError('Selecciona archivo'); const formData = new FormData(); formData.append('backupFile', file); const res = await fetch(`${API_URL}/backup/restore?token=${tokenGuardado}`, { method: 'POST', body: formData }); if(res.ok) showSuccess('Restaurado con éxito'); else showError('Error en restauración'); }
async function handleDescargarExcel() { window.location.href = `${API_URL}/excel/download/clientes?token=${tokenGuardado}`; }
async function handleRestaurarExcel(e) { e.preventDefault(); const file = document.getElementById('excel-file').files[0]; if(!file) return showError('Selecciona archivo'); const formData = new FormData(); formData.append('excelFile', file); const res = await fetch(`${API_URL}/excel/restore/clientes?token=${tokenGuardado}`, { method: 'POST', body: formData }); if(res.ok) { showSuccess('Carga Masiva Completada'); cargarListaClientes(); } else showError('Error en carga'); }
async function handleDescargarExcelVehiculos() { window.location.href = `${API_URL}/excel/download/vehiculos?token=${tokenGuardado}`; }
async function handleRestaurarExcelVehiculos(e) { e.preventDefault(); const file = document.getElementById('excel-file-vehiculos').files[0]; if(!file) return showError('Selecciona archivo'); const formData = new FormData(); formData.append('excelFile', file); const res = await fetch(`${API_URL}/excel/restore/vehiculos?token=${tokenGuardado}`, { method: 'POST', body: formData }); if(res.ok) { showSuccess('Carga Masiva Completada'); cargarConfigMarcas(); } else showError('Error en carga'); }
async function handleDescargarExcelProductos() { window.location.href = `${API_URL}/excel/download/productos?token=${tokenGuardado}`; }
async function handleRestaurarExcelProductos(e) { e.preventDefault(); const file = document.getElementById('excel-file-productos').files[0]; if(!file) return showError('Selecciona archivo'); const formData = new FormData(); formData.append('excelFile', file); const res = await fetch(`${API_URL}/excel/restore/productos?token=${tokenGuardado}`, { method: 'POST', body: formData }); if(res.ok) { showSuccess('Carga Masiva Completada'); cargarInventario(); } else showError('Error en carga'); }
...
function handleUpdateCliente(e) { e.preventDefault(); showSuccess('Cliente Actualizado'); closeTodosModales(); cargarListaClientes(); }
function handleUpdateVehiculo(e) { e.preventDefault(); showSuccess('Vehículo Actualizado'); closeTodosModales(); }
function handleUpdateMantenimiento(e) { e.preventDefault(); showSuccess('Mantenimiento Actualizado'); closeTodosModales(); }
window.deleteCliente = async (id) => { if(await showConfirm('¿Borrar Cliente?')) { await fetch(`${API_URL}/clientes/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); cargarListaClientes(); }};
window.deleteVehiculo = async (id) => { if(await showConfirm('¿Borrar Vehículo?')) { await fetch(`${API_URL}/vehiculos/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); cargarVehiculosCliente(clienteSeleccionadoId); }};
window.deleteMantenimiento = async (id) => { if(await showConfirm('¿Borrar Mantención?')) { await fetch(`${API_URL}/mantenimientos/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${tokenGuardado}` } }); handleBuscarMantenciones(null); }};
